  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">DevSecOps Governance, Policy Engine & Trivy Scans</h1>
          <p className="text-sm text-gray-400">Enterprise management console for security.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card p-5 rounded-xl border border-border col-span-2">
          <h2 className="text-base font-semibold text-white mb-4">Active Resources (security)</h2>
          <div className="space-y-3">
            {items.map((item) => (
              <div key={item} className="p-3 bg-background/50 rounded-lg border border-border/60 flex items-center justify-between">
                <div>
                  <div className="font-semibold text-sm text-gray-200">security-resource-0{item}</div>
