...
class EnvelopeCryptoService:
    ...
    def encrypt_secret(self, plaintext: str, associated_data: Dict[str, Any] = None) -> Dict[str, str]:
        ...
        dek_tag = hmac.new(self.master_key, raw_enc_dek + b"dek-envelope", hashlib.sha256).digest()
    ...
    def decrypt_secret(self, encrypted_payload: Dict[str, str], associated_data: Dict[str, Any] = None) -> str:
        ...
        if not hmac.compare_digest(dek_tag, hmac.new(self.master_key, raw_enc_dek + b"dek-envelope", hashlib.sha256).digest()):
            raise ValueError("Integrity check failed on Master Key envelope")
        ...
        if not hmac.compare_digest(tag, hmac.new(dek, raw_cipher + ad_bytes, hashlib.sha256).digest()):
            raise ValueError("Integrity check failed on ciphertext")

        return self._xor_cipher(raw_cipher, dek, nonce).decode("utf-8")
